...
{
    public static async Task<PagedResult<IEnumerable<TDestination>>> PaginationAsync<TSource, TDestination>(this IQueryable<TSource> querable, int pageSize, int pageNumber)
        where TSource : class
        where TDestination : class
    {
        var count = querable.Count();
        var pageResult = new PagedResult<IEnumerable<TDestination>>
        {
        ...
        pageResult.PageItems = sourceList.Select(item => item as TDestination);
        return pageResult;
    }
}